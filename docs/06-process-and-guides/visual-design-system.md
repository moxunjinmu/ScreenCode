...
![Neo Utility 暖灰近黑暗色仿真设计稿](assets/neo-utility-prototype-dark-v1.png)

暗色稿由可运行仿真页在 1672×941 视口直接输出，采用暖灰近黑表面、酸性青柠主操作和
柔紫 AI 状态。它与亮色稿共用布局、组件与状态模型，不维护独立页面分支。

### 0.4 Neo Utility 亮色主题（已确认，2026-08-03）

![Neo Utility 暖纸白亮色仿真设计稿](assets/neo-utility-prototype-light-v1.png)
...
- 酸性青柠仅用于主操作和选中态；柔紫仅用于 AI 状态；采集状态使用独立成功色。
...
改为 **Neo Utility 中性实色分层 + 酸性青柠主操作 + 柔紫 AI 状态**。
...
主工作区使用 1px 分隔线形成结构，不把每个区域包装成独立大圆角卡片。预览内容始终保留
深色采集源表面，避免应用主题改变被采集画面的含义。
...
`--text-dim` 只允许用于禁用态、占位符和非关键装饰信息；真实状态、计数和可操作标签至少使用
`--text-muted`。两套主题的正文和交互标签均以 WCAG AA 为最低验收标准。

### 3.3 强调色与语义色

```css
--accent: #b9f227; /* 主操作、帧选中、当前标签 */
--accent-ink: #171714;
--ai: #7c5cfc;     /* 亮色；暗色提升为 #9b87ff */
--success: #65a30d;/* 亮色；暗色提升为 #9ad84a */
```

酸性青柠只表达当前选择和主流程动作，不能用于普通装饰或所有按钮。柔紫只表达 AI 在线状态、
AI 标签和对话附件；采集状态、校验成功、警告与错误使用各自语义色。
