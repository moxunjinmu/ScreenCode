...
1. **添加 OpenAI SDK 支持**
   ...
   - 自动检测 API 格式并选择正确的 SDK

2. **支持智谱 AI GLM-5 模型**
   - 确认 GLM-5 模型 ID 为 `glm-5` (小写)
   - 配置智谱 AI Coding Plan 端点
   - 测试 API 连接成功

3. **重构配置系统**
   - 支持每个供应商独立配置 (API Key, Base URL, Model)
   - 添加 `activeProvider` 标识当前激活供应商
   - 添加 `providerConfigs` 按供应商 ID 分组配置
   - 向后兼容旧配置格式
...
4. **增强设置界面**
   ...
   - 每个供应商专属配置面板
   - 输入框 ↔ JSON 双向同步
   - 新增 Base URL, Max Tokens, Temperature 配置项
...
| # | 问题 | 影响 | 状态 | 详情 |
|---|------|------|------|------|
| 1 | `window.electronAPI` 未定义 | 🔴 严重 - 应用无法使用 | 🔴 待修复 | [问题报告 #001](./ISSUE_REPORT_001.md) |
...
| **GLM-5 支持** | ✅ 智谱 AI GLM-5 模型 |
| 第三方中转 | 智谱 AI / OpenRouter / 自定义 Base URL |
| 供应商管理 | 添加/删除/选择供应商 |
| 聊天对话 | 右侧可拖拽面板，支持多图 OCR |
| **双 SDK 支持** | ✅ Anthropic SDK + OpenAI SDK 自动切换 |

### ✅ 配置系统 (2026-02-27)

| 功能 | 说明 |
|------|------|
| **供应商独立配置** | ✅ 每个供应商独立的 API Key / Model 配置 |
| **配置迁移** | ✅ 自动迁移旧配置格式 |
| **双向同步** | ✅ 输入框 ↔ JSON 编辑器实时同步 |
